...
**Current state:** 后端 10/10 阶段全部完成，前端 9 全部完成。系统功能完整（auth / pharmacy / customer / consultation / billing / hospitalization / boarding / Celery PDF 导出 / Redis 缓存 / MinIO 文件上传）。PostgreSQL schema（`backend/sql/init_db.sql`）为数据权威源。
